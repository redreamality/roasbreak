...
> 状态同步：2026-08-21，依据功能基准 `dc8d676`、`content/content-inventory.json`、人工审查账本、内容校验和 E2E。`[x]` 表示仓库产物与现有验收证据均已存在；条件已评估但实现暂缓的条目仍保留 `[ ]`。
...
- [x] 完成 CNT-017 与 CNT-018 的启动评估：当前没有 Search Console、工具聚合或重复用户问题证据，因此保持暂缓，不凭“热门赛道”猜测顺序。
...
- [ ] 启动 CNT-022 模板下载/CSV 导入指南；前置评估已完成，工具尚无 CSV 输入、校验和错误恢复能力，继续暂缓。
...
4. **只按证据重开候选**：CNT-017、CNT-018、CNT-019 至 CNT-022 及 Backlog 剩余 P1/P2 仅在各自重开门槛满足后启动，不以继续发布作为默认动作。
