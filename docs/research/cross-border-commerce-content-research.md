...
- [ ] 完成 CNT-001：用统一模板审计并升级现有 5 篇指南。
- [ ] 完成 CNT-002：发布方法论页，公开收入、成本、归因、舍入和来源规则。
- [ ] 为指南增加可见的最后审校日期、来源列表、适用范围和更正入口。
- [ ] 将 `/guides/` 从平铺列表改为五个主题支柱；实现时若改变交互，按项目规则补相应 E2E。
- [ ] 建立来源清单字段：URL、来源所有者、访问日期、适用地区、影响页面、复核状态。

### 阶段 B：首批高意图内容

- [ ] 发布 CNT-003（公式总览）、CNT-004（POAS vs ROAS）和 CNT-006（Shopify 收入口径）。
- [ ] 发布 CNT-007（Amazon break-even ACoS）和 CNT-008（Google tROAS 利润目标）。
- [ ] 在每篇页面验证“指南 -> 工具 -> 结果”的完整路径，不只验证页面可访问。
- [ ] 对相似查询做内容合并检查，防止现有 ROAS vs ACoS、Revenue Basis 被新页面重复覆盖。

### 阶段 C：促销、新客与平台口径

- [ ] 发布 CNT-009、CNT-010，把免邮/折扣/bundle 决策送入 Promotion Profit 或 Scenario Planner。
- [ ] 发布 CNT-011、CNT-012，分别处理 Meta 和 TikTok 的平台归因口径。
- [ ] 发布 CNT-013、CNT-014、CNT-015，形成 CAC 回收与数据成熟度小专题。
- [ ] 用站内搜索词、工具使用路径和用户问题决定 CNT-016 至 CNT-018 的顺序，不凭“热门赛道”猜测。

### 阶段 D：验证后扩展

- [ ] 只有取得透明、可公开的数据后才启动案例系列。
- [ ] 只有平台变更确实影响计算或决策时才写更新日志。
- [ ] 只有出现持续区域需求且能维护一手来源时才建立国家/区域内容。
