...
调研启动时，ROAS Break 已经拥有 6 个盈利决策工具和 5 篇口径指南。建议把定位收窄为 **Ecommerce Profit Decision Library（电商利润决策库）**：每个页面只回答一个可计算的经营问题，以官方定义和透明公式为依据，用可恢复的场景或计算器承接下一步。优先建设“利润口径、渠道指标翻译、促销/定价、CAC 回收、归因与数据成熟度”五个内容系列，不投入需要每日追更的泛新闻。
...
**2026-08-21 当前状态（`3e34f5a`）**

- 工具仍保持 6 个，已发布内容扩展为 22 篇指南和 1 个方法论资产。
- 五主题导航、可见审校与适用范围、平台工作流、统一方法论、来源治理和指南到工具的可恢复结果路径均已落地并通过门禁；原调研起点保留用于说明本轮建设增量。
...
> 状态同步：2026-08-21，依据功能基准 `3e34f5a`、`content/content-inventory.json`、人工审查账本、内容校验和 E2E。`[x]` 表示仓库产物与现有验收证据均已存在；条件已评估但实现暂缓的条目仍保留 `[ ]`。
...
- [x] 对相似查询做内容合并检查，防止现有 ROAS vs ACoS、Revenue Basis 被新页面重复覆盖；自动唯一性门禁与逐资产语义意图审查均已落账，`NEW-003` 和 `NEW-006` 采用合并而非新增竞争 URL。

### 阶段 C：促销、新客与平台口径

- [x] 发布 CNT-005：新客 ROAS vs blended ROAS。
- [x] 发布 CNT-009：免邮利润阈值。
- [x] 发布 CNT-010：折扣 vs bundle 利润。
- [x] 发布 CNT-011：Meta 广告 ROAS 与归因。
- [x] 发布 CNT-012：TikTok Shop ROAS 与归因。
- [x] 发布 CNT-013：CAC payback cohort 数据准备。
- [x] 发布 CNT-014：转化延迟与数据成熟度。
- [x] 发布 CNT-015：退款与 conversion adjustments。
- [x] 将 CNT-016 合并到 Ecommerce Variable-cost Checklist：覆盖 marketplace/payment、商品、履约与退货成本的字段归属，但不维护易过期的费率大全。
- [x] 完成 CNT-017 启动评估：边际 ROAS 与增量利润计算已并入 Scenario Planner，但独立指南没有 Search Console、工具聚合或重复用户问题证据，继续暂缓。
- [x] 完成 CNT-018 启动评估：当前没有持续区域需求或税费维护责任人，不凭“热门赛道”猜测顺序。
- [x] 完成 Backlog `P1-001` 至 `P1-010` 的本地实现与验证：复用现有 canonical 和决策工具交付免邮、bundle、退货、新客、对账、费用、季节促销、月度预算、扩量守门和 Shopify 商品利润手工流程；全部交互均有桌面/移动 E2E，且不把本地实现写成真实需求或使用证据。

### 阶段 D：验证后扩展

- [ ] 启动 CNT-019 透明案例系列；前置评估已完成，当前没有透明、可公开且获授权的真实数据，继续暂缓。
- [ ] 启动 CNT-020 平台规则更新日志；前置评估已完成，当前没有需要独立成篇且明确影响计算/决策的平台变更，继续暂缓。
- [ ] 启动 CNT-021 国家/区域内容；前置评估已完成，当前没有持续区域需求或当地一手来源维护责任人，继续暂缓。
- [ ] 启动 CNT-022 模板下载/CSV 导入指南；手工字段契约、连接键、ratio-of-sums 和 URL 映射已完成，但仍缺稳定使用数据、文件限制、校验/错误恢复及独立隐私安全评审，继续暂缓。

完整证据、未满足条件与重开门槛记录在 [`conditional-expansion-evaluation.md`](../product/conditional-expansion-evaluation.md)。

### 8.1 后续执行计划

1. **完成本轮生产验证**：部署当前提交后运行 `pnpm production:smoke` 与浏览器模式；生产 sitemap、canonical、schema、链接、布局和 CTA 状态全部通过前，`RELEASE-002` 保持阻断。
2. **收集而非臆测真实信号**：GSC 提供索引、查询、曝光和点击；已同意 Analytics 的会话提供按 guide 归因的 tool click、首次有效计算与成功复制。没有权限或数据时必须记录 unavailable，不能写成 0。
3. **执行 30/60/90 复盘**：用 `pnpm review:check` 跟踪 UTC 到期日；第一批 30 天检查点从 2026-09-12 起到期，每次必须选择 expand、rewrite、merge 或 observe 并保存证据。
4. **只按证据重开候选**：CNT-017、CNT-018、CNT-019 至 CNT-022 及 Backlog P2 仅在各自重开门槛满足后启动，不以继续发布作为默认动作。
