...
> 状态同步：2026-08-21，依据 `main` 分支 `8fd3679`、`content/content-inventory.json`、内容校验和 E2E。`[x]` 表示仓库产物与现有验收证据均已存在；部分完成仍保留 `[ ]`，并在条目中说明剩余工作。

### 阶段 A：内容基础设施

- [x] 完成 CNT-001：用统一模板审计并升级原有 5 篇指南。
- [x] 完成 CNT-002：发布方法论页，公开收入、成本、归因、舍入和来源规则。
- [ ] 为指南增加可见的最后审校日期、来源列表、适用范围和更正入口。**部分完成：** 已发布指南已有审校日期、来源列表和更正入口；仍需把适用范围/地区统一为可见且可校验的字段。
- [x] 将 `/guides/` 从平铺列表改为五个主题支柱，并补充主题导航及移动端 E2E。
- [ ] 建立来源清单字段：URL、来源所有者、访问日期、适用地区、影响页面、复核状态。**部分完成：** 资产清单已有来源 URL、影响页面、审校日期、复核到期日和发布状态；仍缺逐来源所有者、适用地区与独立复核状态。
...
- [ ] 在每篇页面验证“指南 -> 工具 -> 结果”的完整路径，不只验证页面可访问。**部分完成：** Shopify 与 Google 指南已有结果恢复 E2E；其他已发布指南仍只有 CTA、元信息或页面可用性覆盖。
- [ ] 对相似查询做内容合并检查，防止现有 ROAS vs ACoS、Revenue Basis 被新页面重复覆盖。**部分完成：** 资产清单已记录唯一 `primaryIntent`；仍缺发布前的显式意图审查记录或自动化门禁。
...
1. **先关闭 P0 质量缺口**：补齐适用范围/地区和逐来源治理字段；为全部已发布指南补“指南 -> 工具 -> 结果”E2E；把意图审查、生产 smoke 和发布检查保存为可复核记录。
2. **完成剩余核心决策内容**：先做 CNT-003、CNT-004 和 CNT-005，建立公式总览、利润指标边界及新客口径；再做 CNT-009、CNT-010 和 backlog 中的折扣/贡献 LTV 条目，补齐促销与客户经济主题。
